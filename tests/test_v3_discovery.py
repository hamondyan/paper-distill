...
def test_discover_papers_ignores_approved_token_in_frontmatter_only(
    ...
    inbox_dir = tmp_path / "inbox"
    inbox_dir.mkdir(parents=True)
    note_path = inbox_dir / "frontmatter-only.md"
    note_text = (
        "---\n"
        'paper_id: "arxiv:1706.03762"\n'
    ...
    canonical_path = inbox_dir / paper_filename(
        "Attention: Is All You Need",
        "arxiv:1706.03762",
    )
    stub_text = canonical_path.read_text(encoding="utf-8")
    assert "#approved" not in stub_text
...
def test_discover_papers_preserves_existing_unapproved_inbox_note(
    tmp_path, monkeypatch
) -> None:
    inbox_dir = tmp_path / "inbox"
    inbox_dir.mkdir(parents=True)
