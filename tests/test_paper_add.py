...
class PaperAddTest(unittest.TestCase):
    ...
    def test_add_paper_writes_source_evidence_and_canonical_wiki_page(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Wiki page is not created during ingest — it is generated by /compile
            self.assertFalse(Path(result["wiki_paper_abs_path"]).exists())
            ...
            self.assertEqual(wiki_papers["stats"]["papers"], 0)
