...
Paper Distill is an Obsidian-friendly v3 research knowledge system for chat-only discovery, direct paper capture, reading through QMD CLI, and maintaining canonical Markdown notes.
...
```text
discover -> ingest resolved arXiv references -> qmd query/get -> write -> lint
```
